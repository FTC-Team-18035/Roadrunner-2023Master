...
//DEACTIVATED
               ...
               /* if(gamepad2.left_trigger == 1){
                ...
                // }
                if (gamepad2.dpad_up && LiftTarget < MAX_LIFT_HEIGHT - 30) {//If the dpad up is pressed and the lift target is not above the max height
                    LiftTarget = LiftTarget + 30;//This adds lift target +10 to the current lift target having the lift move up slowly
                }
                if (gamepad2.dpad_down && LiftTarget >= 30) {//If the dpad down is pressed and the lift target is higher than 10
                    LiftTarget = LiftTarget - 30;//This subtracts lift target -10 from the current lift target having the lift move up slowly
                    Manual = true;
                }

                // issue lift power for movement
                if (RightLiftMotor.getCurrentPosition() > 100 && RightLiftMotor.getCurrentPosition() < 1900 && ArmActive && Manual == false) {//if the lift position is > 100 and the lift position is < 1900 and the arm is active
                ...
                } else if (RightLiftMotor.getCurrentPosition() >= 1950 && ArmActive && Manual == false) {//If the lift position is > or = to 1950 and the arm is active
                    ArmRotationMotor.setTargetPosition(880);//This swings the arm forwards to delivery position
                    ArmRotationMotor.setPower(1);//Sets the power so the arm will actually turn
                } else if (ArmActive && Manual == false){//If none of the if statements have all of their conditions met and the arm is active
                    ArmRotationMotor.setTargetPosition(0);//Sets the arm back to intake position
                    ArmRotationMotor.setPower(1);//Sets the power so the arm will actually turn
                } else if (Manual == true && RightLiftMotor.getCurrentPosition() > 1000) {
                ...
                telemetry.addData("Manul Override?", Manual);
